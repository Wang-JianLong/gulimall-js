  ...
  <div class="mod-config">
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
      ...
      <el-table-column prop="showStatus" header-align="center" align="center" label="显示状态[0-不显示；1-显示]">
        <template slot-scope="scope">
          <el-switch :active-value="1" :inactive-value="0" v-model="scope.row.showStatus" @change="updateBranchStatus(scope.row)" active-color="#13ce66"
...
import { updateStatus } from '../../../api/product/branch'
export default {
  data() {
    return {
      dataForm: {
  ...
  methods: {
    updateBranchStatus({ brandId, showStatus }) {
      const value = showStatus ? 1 : 0
      updateStatus({ brandId, showStatus: value }).then(() => { this.$message.success('修改成功') })
    },
    // 获取数据列表
    getDataList() {
